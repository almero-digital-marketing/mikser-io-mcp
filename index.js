        ...
        async () => ({
            content: [{
                ...
                text: JSON.stringify({
                    ...
                    // means little until you know a plugin called
                    // mikser-io-drive exists and what it is for.
                    //
        ...
        mcp.simpleTool(
            ...
            async ({ id, include, expand, verbosity = 'full' }) => {
                try {
                    ...
                    if (entity?.uri) {
                        try {
                            entity.diskChecksum = await fileChecksumOf(entity.uri)
        ...
        mcp.simpleTool(
            ...
            async ({ id, collection, relativePath, content = '', ifChecksum, await: awaitCycle, dryRun }) => {
                try {
                    ...
                    // The guard itself is the engine's: containment, the
                    // checksum precondition, advisories, sibling collisions
                    // and the dry-run blast radius are properties of the
                    ...
                    // client sees it as one. `checksum-mismatch` does NOT: it
                    // is a normal outcome carrying the value to retry with,
                    // and burying that in an error string loses it.
                    ...
                    if (!result.ok) {
                        return ok({
                            ...result,
                            hint: `${result.hint} mikser_read_entity reports the catalog checksum, which is only `
                                + 'current as of the last build and may keep returning the value that was just '
                                + 'refused — use `currentChecksum` from THIS response.',
                        })
                    }
                    return ok(result)
                } catch (err) {
