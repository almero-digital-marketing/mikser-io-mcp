...
- **Built-in tools**
  ...
  - *Finding things* — `mikser_search` locates a string across entity meta, source files, and (with `in: ["output"]`) the BUILT files, reporting occurrences per page. That is how you find content you can only describe by what it says, and how you size a change to anything shared before making it.
  - *Working backwards* — `mikser_which` takes a built destination and returns the source that produced it, with the line that defines a given CSS selector. The engine's own `refClosure` supplies the answer, so a bundle assembled from a catalog query resolves to the actual parts that went into it.
